...
Elm.Main.make = function (_elm) {
   ...
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $List = Elm.List.make(_elm);
   var gridLine = F2(function (a,
   b) {
      return $Graphics$Collage.path(_L.fromArray([a
                                                 ,b]));
   });
   var boardHeight = 200;
   var rowMin = (boardHeight / 2 | 0) * -1;
   var rowMax = boardHeight / 2 | 0;
   var boardWidth = 150;
   var columnMin = (boardWidth / 2 | 0) * -1;
   var rows = 4;
   var rowHeight = boardHeight / rows | 0;
   var rowSpacing = rowHeight / 2 | 0;
   var calcY = function (rowNo) {
      return rowMin + (rowNo * rowHeight + rowSpacing);
   };
   var columns = 3;
   var columnWidth = boardWidth / columns | 0;
   var columnSpacing = columnWidth / 2 | 0;
   var getCol = function (colNum) {
      return $Graphics$Collage.path(_L.fromArray([{ctor: "_Tuple2"
                                                  ,_0: $Basics.toFloat(columnMin + columnWidth * colNum)
                                                  ,_1: $Basics.toFloat(rowMin)}
                                                 ,{ctor: "_Tuple2"
                                                  ,_0: $Basics.toFloat(columnMin + columnWidth * colNum)
                                                  ,_1: $Basics.toFloat(rowMax)}]));
   };
   var drawCols = function (colNums) {
      return A2($List.map,
      function (n) {
   ...
   var calcX = function (columnNo) {
      return columnMin + (columnNo * columnWidth + columnSpacing);
   };
   var calcColumnNum = function (index) {
      return _U.cmp(index,
      columns) < 0 ? index : A2($Basics._op["%"],
      index,
      columns);
   };
   var getMoveX = function (index) {
      return $Basics.toFloat(calcX(calcColumnNum(index)));
   };
   var calcRowNum = function (index) {
      return _U.cmp(index,
      columns) < 0 ? 0 : index / columns | 0;
   };
   var getMoveY = function (index) {
      return $Basics.toFloat(calcY(calcRowNum(index)));
   };
   var placePiece = F2(function (index,
   pieceValue) {
      return A2($Graphics$Collage.move,
      {ctor: "_Tuple2"
      ,_0: getMoveX(index)
      ,_1: getMoveY(index)},
      $Graphics$Collage.toForm($Graphics$Element.show(pieceValue)));
   });
   var placePieces = function (l) {
      return $Array.toList(A2($Array.indexedMap,
      placePiece,
      $Array.fromList(l)));
   };
   var model = _L.fromArray([4
                            ,5
                            ,6
                            ,0
                            ,0
   ...
   _elm.Main.values = {_op: _op
                      ...
                      ,getMoveX: getMoveX
                      ,getMoveY: getMoveY
                      ,calcColumnNum: calcColumnNum
                      ,calcRowNum: calcRowNum
                      ,calcX: calcX
                      ,calcY: calcY
